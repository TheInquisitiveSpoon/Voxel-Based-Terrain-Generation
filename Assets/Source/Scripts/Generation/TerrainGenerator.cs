...
public class TerrainGenerator : MonoBehaviour
{
    ...
    public ChunkData GenerateChunk(ChunkData data)
    {
        ...
        for (int x = 0; x < data.Width; x++)
        {
            for (int z = 0; z < data.Width; z++)
            {
                ...
                Biome biomeSelector = GetBiomeGenerator(new Vector3Int(data.WorldPos.x + x, 0, data.WorldPos.z + z), data);
    ...
    //  biomes borders.
    private Biome GetBiomeGenerator(Vector3Int worldPos, ChunkData data)
    {
        ...
        return new Biome(biome1, Mathf.RoundToInt(groundLevel1 * weight1 + groundLevel2 * weight2));
    }
    ...
    private BiomeGenerator SelectBiome(int index)
    {
        ...
        foreach (BiomeGenerator biomeGenerator in BiomeGenerators)
        {
            if
            (
                temperature     >= biomeGenerator.MinTemperature    &&
                temperature     < biomeGenerator.MaxTemperature     &&
                precipitation   >= biomeGenerator.MinPrecipitation  &&
                precipitation   < biomeGenerator.MaxPrecipitation
            )
            {
    ...
    public void GetBiomeCenters(Vector3 playerPos, int ChunkRenderDist, int ChunkWidth)
    {
        ...
        BiomeCenters = BiomeCenterCalculator.CalculateBiomeCenters(playerPos, ChunkRenderDist, ChunkWidth);
